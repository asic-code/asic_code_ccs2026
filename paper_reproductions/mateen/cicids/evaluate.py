...
from data_loader import load_ids17, sanity_print
...
def main() -> None:
    ...
    ap.add_argument("--csv", type=str, default=None,
                    help="path to clean_data.csv; auto-detected if omitted")
    ap.add_argument("--out", type=str, default="out/results.json")
    args_cli = ap.parse_args()

    print(f"DEVICE = {device_mod.DEVICE_STR}")
    t0 = time.time()
    split = load_ids17(fraction=1.0, seed=args_cli.seed, csv_path=args_cli.csv)
    sanity_print(split)
