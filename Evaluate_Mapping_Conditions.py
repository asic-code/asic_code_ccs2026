...
def run_evaluation_for_condition(
    ...
    # For decision/filter stages, prefer virtual cluster labels over ground-truth label.
    def _select_filter_target_col(df):
        for col in ('cluster', 'adjusted_cluster', 'label'):
            if col not in df.columns:
                continue
            vals = pd.Series(df[col]).dropna().to_numpy()
            if vals.size == 0:
                continue
    ...
    train_filter_col = _select_filter_target_col(base_mapped_train)
    test_filter_col = _select_filter_target_col(base_mapped_test)
    if train_filter_col is None:
        raise ValueError("No usable binary filter column found in train data (expected one of: cluster, adjusted_cluster, label).")
    if test_filter_col is None:
        raise ValueError("No usable binary filter column found in test data (expected one of: cluster, adjusted_cluster, label).")
    print(f"[INFO] Filter target columns: train='{train_filter_col}', test='{test_filter_col}'")
    ...
    train_normal_base = base_mapped_train[base_mapped_train[train_filter_col] == 0].drop(
        columns=['label', 'cluster', 'adjusted_cluster', 'original_label'],
        errors='ignore'
    )
    ...
    if 'original_label' in test_df_base.columns:
        ...
        test_normal_virtual_base = base_mapped_test[base_mapped_test[test_filter_col] == 0].drop(
            columns=['label', 'cluster', 'adjusted_cluster', 'original_label'],
            errors='ignore'
        )
    ...
    regrouped_normal = base_mapped_train[base_mapped_train[train_filter_col] == 0].drop(
        columns=['label', 'cluster', 'adjusted_cluster', 'original_label'],
        errors='ignore'
    )
...
def main():
    ...
    # Virtual labeling is wired in main() (cached test set); no separate --use_virtual_labeling flag here.
    ...
    # Initial n_clusters; main() may replace from hardcoded_k_combinations / VL branch.
    ...
    if args.file_type in ['CICIoT2023', 'CICIoT'] and args.sample_10pct:
        sample_frac = 0.1
        print(f"\n[INFO] SAMPLING: Randomly sampling {sample_frac * 100}% of the {args.file_type} dataset.")
    ...
    whitelist_signatures = create_whitelist(
        ...
        # precision_threshold=0.5  # optional; not passed
    )
