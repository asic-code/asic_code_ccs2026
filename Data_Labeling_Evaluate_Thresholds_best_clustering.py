...
# 1. Known-normal reference
# - Switched to global reference + consistency alignment
# - Added build_consistent_known_normals(...)
#   * Sample 90% normal indices from current (X, labels) with fixed seed
#   * Prefer feature-space consistent normals; fallback to global reference

# 2. Preprocessing pipeline
# - Uses scaling_label_encoding-focused pipeline
#   time_scalar_transfer → choose_heterogeneous_method('scaling_label_encoding') → PCA(conditional)

# 3. No label leakage
# - No GT used as training signal
# - Known-normal stays within original CNI-safe design

# 4. Misc
# - No unrelated changes
# - Passed syntax & lint checks
# - n_splits can be exposed via argparse (optional)
'''
...
from Clustering_Method.clustering_nomal_identify import clustering_nomal_identify, create_global_reference_normal_samples
from Clustering_Method.cluster_refinement import refine_clusters
from Modules.refine_utils import get_default_refine_config, load_refine_config_from_json
...
def get_cache_path(file_type, file_number, clustering_algorithm, refine_enabled=False):
    """Generates a consistent path for cache files."""
    refine_suffix = "_refine" if refine_enabled else ""
    filename = f"{file_type}_{file_number}_{clustering_algorithm}{refine_suffix}_clustering_cache.pkl"
...
def build_known_normal_reference_samples(X_reduced, original_labels, fraction=0.90, seed=42):
    """
    Build global known-normal reference samples in the same feature space as X_reduced.
    This is used by CNI to avoid per-call fallback reconstruction.
    """
    X_arr = np.asarray(X_reduced)
    y_arr = np.asarray(original_labels).ravel()
    known_normal_indices = np.where(y_arr == 0)[0]
    if len(known_normal_indices) == 0:
        if X_arr.ndim == 2:
            return np.empty((0, X_arr.shape[1])), known_normal_indices
        return np.array([]), known_normal_indices

    frac = max(0.0, min(1.0, float(fraction)))
    n_pick = max(1, int(len(known_normal_indices) * frac))
    rng = np.random.default_rng(int(seed))
    picked = np.sort(rng.choice(known_normal_indices, size=min(n_pick, len(known_normal_indices)), replace=False))
    return X_arr[picked], known_normal_indices


def build_consistent_known_normals(
    X,
    labels,
    known_normal_fraction,
    seed,
    global_known_normal_samples_pca,
    global_known_normal_indices,
):
    """
    Match best_clustering_selector semantics:
    - Prefer re-sampling known-normal from the CURRENT clustering input X/labels
    - Fall back to global reference if current normals are missing
    """
    X_arr = np.asarray(X)
    y_arr = np.asarray(labels).ravel()
    normal_idx = np.where(y_arr == 0)[0]
    if len(normal_idx) > 1:
        frac = max(0.0, min(1.0, float(known_normal_fraction)))
        sample_size = int(len(normal_idx) * frac)
        if sample_size == 0:
            sample_size = 1
        np.random.seed(int(seed))
        picked_local = np.random.choice(len(normal_idx), size=min(sample_size, len(normal_idx)), replace=False)
        consistent_idx = normal_idx[picked_local]
        consistent_samples = X_arr[consistent_idx]
        return consistent_samples, consistent_idx

    if global_known_normal_samples_pca is not None and len(np.asarray(global_known_normal_samples_pca)) > 0:
        return np.asarray(global_known_normal_samples_pca), np.asarray(global_known_normal_indices if global_known_normal_indices is not None else [])
...
def generate_cache(args, timing_info):
    ...
    try:
        ...
        # best_clustering-style scaling_label_encoding preprocessing
        data_for_processing = time_scalar_transfer(data.copy(), args.file_type)

        # 1) Explicit feature selection (same family as best_clustering)
        ...
        # 2) Numeric cleaning for scaling_label_encoding path:
        #    coerce numeric -> replace inf -> drop NaN rows
        ...
        if columns_to_clean:
            ...
            for col in columns_to_clean:
                if not np.isfinite(data_for_processing[col].values).all():
                    if np.isinf(data_for_processing[col].values).any():
                        ...
                        data_for_processing[col] = data_for_processing[col].replace([np.inf, -np.inf], replacement_val)
            ...
            if rows_with_nan.any():
                ...
                if data_for_processing.empty:
                    print("[ERROR] All rows were removed during numeric cleaning.")
                    return

        original_labels = data_for_processing['label'].to_numpy()
        X_features = data_for_processing.drop(columns=['label'], errors='ignore')
        X_processed, _, _, _ = choose_heterogeneous_method(
            X_features,
            args.file_type,
            'scaling_label_encoding',
            'N',
        )
        pca_want = 'N' if args.file_type in ['DARPA98', 'DARPA', 'CICModbus23', 'CICModbus'] else 'Y'
        if pca_want == 'Y':
            X_reduced = pca_func(X_processed)
        else:
            X_reduced = X_processed.to_numpy() if hasattr(X_processed, 'to_numpy') else np.asarray(X_processed)
        ...
        global_known_normal_samples_pca, global_known_normal_indices = create_global_reference_normal_samples(
            args.file_type,
            args.file_number,
            "scaling_label_encoding",
        )
        known_normal_samples_pca, known_normal_indices = build_consistent_known_normals(
            X_reduced,
            original_labels,
            known_normal_fraction=0.90,
            seed=42,
            global_known_normal_samples_pca=global_known_normal_samples_pca,
            global_known_normal_indices=global_known_normal_indices,
        )
        ...
        try:
            if args.max_score:
                result = comprehensive_optimization_max_score_label_free(
                    args.clustering,
                    X_reduced,
                    None,
                    args.file_type,
                ...
                # Optional non-max_score fallback path (parser-controlled).
                # Keep labels out of clustering decision path.
                best_params = {'n_clusters': int(args.max_clusters)}
                ...
                print(f"[No MaxScore] Ran single clustering with params={best_params}")
            ...
            if args.refine_clusters and raw_labels is not None:
                try:
                    raw_labels = refine_clusters(
                        X_reduced,
                        np.asarray(raw_labels).ravel(),
                        config=args.runtime_refine_config,
                        protected_indices=known_normal_indices,
                    )
                    print("[Refine] Applied refine_clusters to raw labels.")
                except Exception as e:
                    print(f"[Refine WARN] refine_clusters failed and was skipped: {e}")
            ...
            cache_data = {
                ...
                'global_known_normal_samples_pca': known_normal_samples_pca,
                'known_normal_indices': np.asarray(known_normal_indices),
                'refine_applied': bool(args.refine_clusters),
            ...
            cache_path = get_cache_path(
                ...
                args.file_number,
                args.clustering,
                refine_enabled=bool(args.refine_clusters),
            )
...
def _evaluate_single_threshold(args_tuple):
    ...
    threshold, X, original_labels, raw_labels, num_clusters, global_known_normal_samples_pca, known_normal_indices = args_tuple
    ...
    try:
        ...
        cni_result = clustering_nomal_identify(
            ...
            known_normal_idx=np.asarray(known_normal_indices).ravel()
        )
...
def evaluate_thresholds(args, timing_info):
    ...
    global_known_normal_samples_pca = cached_data.get('global_known_normal_samples_pca', None)
    known_normal_indices = cached_data.get('known_normal_indices', None)
    if known_normal_indices is None:
        known_normal_indices = np.where(original_labels == 0)[0]
    if global_known_normal_samples_pca is None:
        global_known_normal_samples_pca, _ = build_known_normal_reference_samples(
            X_for_identification,
            original_labels,
            fraction=0.90,
            seed=42,
        )
        print(f"[KnownNormal] Cache has no global reference. Rebuilt with shape={np.asarray(global_known_normal_samples_pca).shape}.")
    else:
        print(f"[KnownNormal] Loaded global reference from cache with shape={np.asarray(global_known_normal_samples_pca).shape}.")

    if args.refine_clusters and not bool(cached_data.get('refine_applied', False)):
        try:
            raw_cluster_labels = refine_clusters(
                X_for_identification,
                np.asarray(raw_cluster_labels).ravel(),
                config=args.runtime_refine_config,
                protected_indices=np.asarray(known_normal_indices).ravel(),
            )
            print("[Refine] Applied refine_clusters on cached labels before threshold evaluation.")
        except Exception as e:
            print(f"[Refine WARN] refine_clusters during evaluate_thresholds failed and was skipped: {e}")
    ...
    tasks = [
        (thresh, X_for_identification, original_labels, raw_cluster_labels, num_clusters, global_known_normal_samples_pca, known_normal_indices)
...
def main():
    ...
    parser.add_argument('--max_score', action='store_true', default=True,
                        help="Use Max Score optimization for clustering (default: enabled).")
    parser.add_argument('--no_max_score', dest='max_score', action='store_false',
                        help="Disable Max Score and run single clustering with --max_clusters.")
    parser.add_argument('--refine_clusters', action='store_true',
                        help="Enable optional cluster refinement before threshold evaluation.")
    parser.add_argument('--refine_config_json', type=str, default=None,
                        help="Optional refine config JSON path.")
    parser.add_argument('--refine_second_gate', action='store_true',
                        help="Enable refine second gate (implies --refine_clusters).")
    ...
    args.runtime_refine_config = get_default_refine_config()
    if args.refine_config_json:
        try:
            args.runtime_refine_config = load_refine_config_from_json(args.refine_config_json)
            if not args.refine_clusters:
                args.refine_clusters = True
            print(f"[Refine] Loaded config from {args.refine_config_json}")
        except Exception as e:
            print(f"[Refine WARN] Failed to load refine config json: {e}. Using default config.")
    if args.refine_second_gate:
        args.runtime_refine_config["second_gate_enabled"] = True
        if not args.refine_clusters:
            args.refine_clusters = True
